    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <div class="panel-heading">任务信息</div>
                <form method="post" action="${pageContext.request.contextPath}/TeacherTask/updateSave" enctype="multipart/form-data">

                    <div class="row data-type">
                        <!-- 任务编号 -->
                        <div class="col-md-2 title">任务编号</div>
                        <div class="col-md-4 data">
                            <select id="homeworkSelect" class="form-control select2" style="width: 100%;" name="homeworkID">
                                <option value="">选择任务</option>
                                <c:forEach var="task" items="${taskLists}">
                                    <option value="${task.homeworkID}">${task.homeworkID}</option>
                                </c:forEach>
                            </select>
                        </div>
                        <!-- 任务标题 -->
                        ...
                        <!-- 上次附件上传 -->
                        <div class="col-md-2 title">上次上传附件</div>
                        <div class="col-md-10 data" style="display: flex; align-items: center;">
                            <!-- 显示文件名 -->
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $('#datetimepicker-due').datetimepicker({
                        format: 'HH:mm:ss', // 设置为日期时间格式
                        showTodayButton: true,          // 显示“今天”按钮
                        sideBySide: true,              // 日期和时间选择并排显示
                        locale: 'zh-cn',               // 设置为中文语言环境
                        autoclose: false,              // 确保不会自动关闭
                        pickDate: false               // 禁止选择日期，只显示时间
                    });
                });
                    $(document).ready(function() {
                    // 初始化 select2 插件
                    ...
                    $("#homeworkSelect").change(function() {
                        ...
                        console.log("选中的 homeworkID:", homeworkID);  // 打印检查

                        if (homeworkID) {
                            $.ajax({
                                url: '${pageContext.request.contextPath}/TeacherTask/toGetUpdateTask',
                                type: 'GET',
                                data: { homeworkID: homeworkID },  // 传递 homeworkID
                                success: function(response) {
                                    console.log("返回的任务数据：", response); // 打印返回数据
                                    ...
                                    // 填充班级选择框 (classID)
                                    $("#classSelect").val(response.classID).trigger('change'); // 触发 select2 的值更新

                                    if (response.lastFile) {
                                        var filePath = response.lastFile; // 文件路径
                                        var fileName = filePath.split('/').pop(); // 提取文件名
                                        var cleanFileName = fileName.replace("teacher-uploads", "");
                                        // 将文件名显示在输入框
                                        $("input[name='lastFile']").val(cleanFileName);

                                        $("#downloadBtn").off("click").on("click", function () {
                                            var cleanFileName = $("input[name='lastFile']").val(); // 获取文件名
                                            if (!cleanFileName) {
                                                alert("文件名不存在！");
                                                return;
                                            }
